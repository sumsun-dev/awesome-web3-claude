...
- **Do your own research** — Always conduct your own security audit before using any tool in production. The `Official` tag means the project/organization officially published it; it does not guarantee security.
- **Smart contract risks** — On-chain transactions are irreversible. Minimize permissions before granting AI agents wallet access, and always test on testnets first.
- **API key management** — Before entering API keys into MCP servers or AI agent tools, review the tool's source code to verify keys are not exfiltrated.
- **AI-generated code** — Smart contract code generated by Claude Code or other AI must be manually reviewed before deployment. AI can introduce subtle security vulnerabilities.
- **Third-party dependencies** — Each tool in this list is an independent third-party project. Verify their licenses, maintenance status, and security track record yourself.
- **Limitation of liability** — The curators of this list accept no responsibility for asset loss, security incidents, or any other damages resulting from the use of listed tools.
...
- [ ] Have you reviewed the source code to confirm it contains no malicious code?
- [ ] Does the tool avoid requesting unnecessary permissions (filesystem, network, wallet access)?
- [ ] Is the project actively maintained with no known security vulnerabilities?

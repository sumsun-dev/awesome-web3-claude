...
│  ├─ 관리 명령어: /add /remove /move /edit /skills     │
│  ├─ POST /api/approve → 채널 메시지 승인 플로우       │
│  │   └─ approve-publish.mjs: 미리보기 → 승인 → 채널   │
│  └─ GitHub workflow_dispatch → update-readme.yml     │
└──────────────────────────┬──────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────┐
│  GitHub Actions (update-readme.yml)                   │
│  ├─ repos.json 수정 (add/remove/move/update-desc/update-skills/skip/keep) │
│  ├─ generate-readme.mjs → README.md + README_EN.md   │
│  ├─ git commit & push                                │
│  ├─ add: POST /api/approve → 관리자 승인 후 채널 발송 │
│  └─ 기타: 직접 채널 + 관리자 DM 알림                  │
└─────────────────────────────────────────────────────┘
```
...
│   └── update-readme.yml       # 승인 후 README 업데이트 (add/remove/move/update-desc/update-skills)
├── scripts/
...
│   ├── generate-readme.mjs     # repos.json → README 생성 (스킬 <details> 포함)
│   ├── skill-utils.mjs         # 스킬 탐색/파싱 공유 유틸 (discoverSkillsInRepo, parseSkillFrontmatter)
│   ├── migrate-skills.mjs      # 스킬 마이그레이션 (일회성)
...
│       ├── callbacks.mjs       # 콜백 핸들러 (discover + command + approve 통합)
...
Claude Code headless 한국어 설명 생성, workflow_dispatch 트리거, 채널 메시지 승인 API, 헬스 체크 제공.

**엔드포인트:**
- `POST /webhook/{BOT_TOKEN}` — Telegram 웹훅 (텍스트 메시지 세션 라우팅 포함)
- `POST /api/describe` — Claude Code headless 한국어 설명 생성
- `POST /api/approve` — 채널 메시지 승인 플로우 트리거 (GitHub Actions에서 호출)
- `GET /health` — 헬스 체크 (localhost만)
...
- `callbacks.mjs` — 콜백 핸들러 (`routeCallback()`) — discover + cmd_* + approve 통합
- `approve-publish.mjs` — 채널 메시지 승인 발행 (스킬 유무 통합, `/skills` + `/api/approve` 진입점)

**채널 메시지 승인 플로우:**
- `add` 시 채널 자동 전송 대신 관리자 DM 미리보기 → 승인 → 채널 발송
- 스킬 있는 레포: AI 추천 → 스킬 토글 → 미리보기 → 승인/수정/스킵
- 스킬 없는 레포: AI 한글 메시지 생성 → 미리보기 → 승인/수정/스킵
- `remove/move/update-desc/update-skills`: 기존 채널 자동 전송 유지

### generate-readme.mjs — README 생성

`repos.json` → `README.md` (한국어) + `README_EN.md` (영어) 자동 생성.
8개 섹션 순서 고정, 동적 스타 배지, 이중언어 설명.
멀티-스킬 레포에는 `<details>` 접이식 스킬 서브테이블 자동 렌더링.

### skill-utils.mjs — 스킬 유틸

GitHub API로 레포 내 SKILL.md 파일 탐색 + YAML frontmatter 파싱.
`discoverSkillsInRepo()`, `parseSkillFrontmatter()`, `countTotalSkills()` 제공.
discover.mjs, migrate-skills.mjs, update-readme.yml에서 공유.
...
```jsonc
{
  "metadata": { "lastUpdated": "...", "totalEntries": 51, "totalSkills": 118 },
  "sections": [{
    ...
    "repos": [{
      ...
      "skills": [{                   // 선택, 멀티-스킬 레포만
        ...
        "description": { "ko": null, "en": "Audits GitHub Actions workflows..." }
      }]
    }]
  }]
}
...
| `TELEGRAM_CHANNEL_ID` | 공개 채널 ID (채널 발행용) |
...
npm run generate        # repos.json → README 생성 (스킬 <details> 포함)
npm run webhook         # VPS Express 서버 실행
npm run migrate         # 초기 마이그레이션 (일회성)
npm run migrate-skills  # 스킬 마이그레이션 (일회성, GITHUB_TOKEN 필요)
```
...
6. **업데이트** — `update-readme.yml` → repos.json 수정 → README 재생성
7. **채널 발행** — add: VPS `/api/approve` → DM 미리보기 → 관리자 승인 → 채널 발송
   - remove/move/update-desc/update-skills: 기존 채널 자동 전송 유지

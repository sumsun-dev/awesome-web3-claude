...
- **Infra**: GitHub Actions + VPS Express 서버 (port 3847)
...
│  │   └─ POST VPS /api/describe → Claude Code haiku   │
│  │       → 한국어 설명 생성 (~5초/건)                 │
...
└──────────────────────────┬──────────────────────────┘
                    ...
                    사용자가 "추가" 클릭
                           │
┌──────────────────────────▼──────────────────────────┐
│  VPS — telegram-webhook.mjs (Express, port 3847)     │
...
| 번역 | VPS `/api/describe`로 한국어 설명 생성 (skip 제외) |
...
| 엔드포인트 | 용도 |
|-----------|------|
| `POST /webhook/{BOT_TOKEN}` | Telegram callback 처리 (add/skip/keep/remove) |
| `POST /api/describe` | Claude Code headless 한국어 설명 생성 (Bearer 인증) |
| `GET /health` | 헬스 체크 |
...
- systemd에 `Environment=HOME=/root` 필수
...
| `VPS_API_URL` | 하드코딩 | `https://technotekorea.com/awc/api/describe` |
...
| `API_SECRET` | `/api/describe` 인증 |
| `PORT` | Express 포트 (3847) |

## VPS 배포

- **서버**: `5.223.60.9` (technotekorea.com)
- **경로**: `/root/awc-webhook/`
- **서비스**: `systemctl restart awc-webhook`
- **nginx**: `/awc/` → `localhost:3847/` (proxy_read_timeout 330s)
- **로그**: `journalctl -u awc-webhook -f`
